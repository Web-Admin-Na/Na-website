...
<div id="main" class="container mb-50">
  <div id="body" class="nine columns">
    ...
    <?php
      if( isset( $_POST['contactSubmit'] ) ) {
        ...
        if ( !empty( $name ) && !empty( $email ) && !empty( $message ) ) {
          ...
          if( $sent ) echo "<p class='success'>Your mail was sent successfully. We will get back to you soon.</p>";
    ...
    <form class="contactus" method="post" action="">
      ...
      <p>
        <label><?php _e('Email *', 'NA'); ?></label>
        <input type="email" name="cemail" required>
      </p>
      <p>
        <label><?php _e('Message', 'NA'); ?></label>
        <textarea name="cmessage" required></textarea>
      </p>
      <input class="btn medium blue" type="submit" name="contactSubmit" value="<?php _e('Send', 'NA'); ?>">
      <p><em>* <?php _e('required fields', 'NA'); ?></em></p>
    </form>
  </div> <!-- #body -->
  <?php get_sidebar(); ?>
</div> <!-- #main -->
